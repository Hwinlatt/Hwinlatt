(function ($) {
    ...
    $(".quantity button").on("click", function () {
        ...
        let remain = parseInt(
            button.parent().parent().find(".rmNumber").html()
        );
...
let addFavLocalStorage = (id) => {
    ...
    if (favs != null) {
        for (let i = 0; i < favs.length; i++) {
            if (favs[i] == id) {
                Swal.fire("Error!", "Already Exists in Favourite!", "error");
                return;
            }
        }
    } else {
    ...
    Swal.fire("Success!", "Add to Favourite Successful.", "success");
    getFavCount();
    FavIconChange(id);
};
//FavIconChange 

let FavIconChange = (id) => {
    id = parseInt(id);
    let favs = JSON.parse(localStorage.getItem('fav'));
    if (favs == null) {
        favs = [];
    ...
    if (favs.includes(id)) {
        $(`button[onclick="addFavLocalStorage(${id})"]`).hide();
        $(`button[onclick="removeFavLocalStorage(${id})"]`).show();
    }else{
        $(`button[onclick="addFavLocalStorage(${id})"]`).show();
        $(`button[onclick="removeFavLocalStorage(${id})"]`).hide();
    }
}
//Call Function when Fabourite Change
let localStoreFavChange = (favs) => {
    localStorage.setItem("fav", JSON.stringify(favs));
    getFavCount();
};
//Get favourite list
